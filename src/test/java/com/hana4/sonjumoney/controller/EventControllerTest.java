...
public class EventControllerTest extends ControllerTest {
	...
	public void getAllEventsDefaultDateTest() throws Exception {
		mockMvc.perform(get("/api/events")
			...
			.andExpect(jsonPath("$[0].current_date").value("2025-01-19"))
			...
			//current_date 기준 두번째 일정
			.andExpect(jsonPath("$[1].event_id").value(1))
			.andExpect(jsonPath("$[1].event_category").value("여행"))
			...
			.andExpect(jsonPath("$[1].current_date").value("2025-01-25"))
			...
			//current_date 기준 다섯번째 일정
			.andExpect(jsonPath("$[4].event_id").value(1))
			.andExpect(jsonPath("$[4].event_category").value("여행"))
			.andExpect(jsonPath("$[4].event_name").value("우리 가족 여행"))
			.andExpect(jsonPath("$[4].start_date_time").value("2025-01-25T10:00:00"))
			.andExpect(jsonPath("$[4].end_date_time").value("2025-01-31T23:59:59"))
			.andExpect(jsonPath("$[4].current_date").value("2025-01-28"))
			.andExpect(jsonPath("$[4].event_participants[0].user_name").value("계좌 있는 놈"))
			.andExpect(jsonPath("$[4].event_participants[1].user_name").value("계좌 없는 놈"))
			.andDo(print());
	...
	public void getAllEventsGetDateTest() throws Exception {
		mockMvc.perform(get("/api/events")
			...
			.andExpect(jsonPath("$[0].event_category").value("기념일"))
			...
			.andExpect(jsonPath("$[0].current_date").value("2025-02-01"))
			.andExpect(jsonPath("$[0].event_participants[0].user_name").value("계좌 있는 놈"))
			.andExpect(jsonPath("$[0].event_participants[1].user_name").value("계좌 없는 놈"))
			.andDo(print());
	...
	public void getEventTest() throws Exception {
		mockMvc.perform(get("/api/events/{event_id}", 2)
			...
			.andExpect(jsonPath("$.event_category").value("약속"))
